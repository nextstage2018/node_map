...
const THEME_STATUS_BADGE: Record<string, { label: string; color: string }> = {
...
export default function ThemeSection({
  ...
  const badge = THEME_STATUS_BADGE[theme.status] || THEME_STATUS_BADGE.active;
  ...
  return (
    <div className="border border-slate-200 rounded-lg bg-white overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 bg-slate-50 group">
        ...
        <span className="text-xs font-bold text-slate-800 truncate">{theme.title}</span>
        <span className={`text-[10px] px-1.5 py-0.5 rounded-full shrink-0 ${badge.color}`}>
          {badge.label}
        </span>
        {theme.description && (
          <span className="text-[10px] text-slate-400 truncate hidden sm:inline">{theme.description}</span>
        )}
        <div className="ml-auto opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <MoreMenu
            items={[
              { label: '編集', icon: <Pencil className="w-3.5 h-3.5" />, onClick: onEditTheme },
      ...
      <DeleteConfirmDialog
        ...
        title={`テーマ「${theme.title}」を削除しますか？`}
        description="配下マイルストーンのテーマ紐づけが解除されます。マイルストーンとタスクは削除されません。"
      />
    </div>
  );
}
