// v4.0: ゴール作成・編集フォーム（モーダル）— 旧 ThemeForm
'use client';
...
export default function GoalForm({ isOpen, onClose, onSubmit, initialData, isLoading = false }: GoalFormProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md mx-4 p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="block text-xs text-slate-600 mb-1">ゴール名 *</label>
            <input
              type="text"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder="例: Phase1: 現状分析"
