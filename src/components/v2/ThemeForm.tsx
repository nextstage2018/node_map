...
export default function ThemeForm({ isOpen, onClose, onSubmit, initialData, isLoading = false }: ThemeFormProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md mx-4 p-6">
        ...
        <h3 className="text-sm font-bold text-slate-800 mb-4">
          {isEdit ? 'テーマを編集' : 'テーマを追加'}
        </h3>

        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="block text-xs text-slate-600 mb-1">テーマ名 *</label>
            <input
              type="text"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder="例: ターゲット再定義"
