...
export default function ContactsPage() {
  ...
  useEffect(() => {
    if (selectedContact) {
      ...
      setOrgName(org?.name || '');
      // Phase 36: AIコンテキストを初期化
      setAiContext(selectedContact.ai_context || '');
  ...
  const analyzeContact = async () => {
    ...
    try {
      ...
      if (data.success && data.data?.ai_context) {
        ...
        setActionResult({ type: 'success', text: 'AI分析が完了しました' });
        // コンタクトリストのai_contextも更新
        setContacts((prev) =>
          prev.map((c) => c.id === selectedContact.id ? { ...c, ai_context: data.data.ai_context, ai_analyzed_at: new Date().toISOString() } : c)
        );
      } else {
        setActionResult({ type: 'error', text: data.error || 'AI分析に失敗しました' });
      }
    } catch {
      setActionResult({ type: 'error', text: '通信エラー' });
    } finally {
                    ...
                    {aiContext ? (
                      ...
                      <div className="flex items-center justify-center h-32 text-slate-400">
                        <div className="text-center">
                          ...
                          <p className="text-xs">AI分析を実行すると、この人物との関係性や<br />コミュニケーション傾向が表示されます</p>
                        </div>
                      </div>
                    )}
                  </div>
