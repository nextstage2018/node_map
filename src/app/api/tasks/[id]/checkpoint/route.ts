...
${bossFeedbackContext ? '- 上長の過去の指摘パターンを踏まえた対応ができているか（※評価コメントでは指摘の具体的内容を伝え、「フィードバックN番」のような番号参照は使わないこと）' : ''}
...
## 改善ヒントの書き方ルール
- ヒントはこのタスクの担当者が読むものです。担当者が次に何をすべきか具体的なアクションで書いてください
- 「上長フィードバックN番」「判断基準N」のような内部データへの参照番号は絶対に使わないでください
- 代わりに、上長の指摘内容を自分の言葉で言い換えて具体的に伝えてください
- 例: ×「上長フィードバック9に基づき詳細化が必要」→ ○「各作業工程を『誰が・何を・どの順で・何分で』のレベルまで分解し、資料の該当セクションに追記してください」
- 例: ×「判断基準3・5・11との整合性を確認」→ ○「競合との差別化ポイントを3つ以上リスト化し、それぞれ『なぜ自社だけが提供できるか』の根拠を1文で添えてください」
- 各ヒントは「〜してください」「〜を追記してください」「〜を検討してください」のように行動指示で終えてください
...
## 出力形式（必ずこのJSON形式で返してください）
{
  ...
  "overall_feedback": "全体的なフィードバック（2-3文。内部参照番号は使わず平易な言葉で）",
  "improvement_hints": ["具体的アクション1（行動指示形式で）", "具体的アクション2", "具体的アクション3"]
}`;
    ...
    // 前回の採点結果を取得（task_conversationsからphase='checkpoint'のものを検索）
    ...
    try {
      ...
      if (prevCheckpoints && prevCheckpoints.length > 0) {
        try {
          previousResult = JSON.parse(prevCheckpoints[0].content);
        } catch { /* 解析失敗は無視 */ }
      }
    } catch { /* ignore */ }
    ...
    const { error: insertErr } = await supabase.from('task_conversations').insert({
      task_id: taskId,
      user_id: userId,
      role: 'assistant',
    ...
    if (insertErr) {
      console.error('[Checkpoint API] 採点結果保存エラー:', insertErr);
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        _saveError: insertErr ? insertErr.message : null,
      },
    });
  } catch (error) {
