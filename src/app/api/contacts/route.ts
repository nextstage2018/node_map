...
export async function GET(request: NextRequest) {
  try {
    ...
    const contacts: {
      ...
      companyName: string; nameReading: string; department: string; notes: string;
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { id, name, nameReading, relationshipType, confirmed, mainChannel, companyName, department, notes } = body;
    ...
    if (nameReading !== undefined) updateData.name_reading = nameReading;
