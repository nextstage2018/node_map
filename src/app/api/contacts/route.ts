...
export async function GET(request: NextRequest) {
  try {
    ...
    // Phase 35: senderからマッチした既存contact_personsのIDを記録
    ...
    const contacts = senders.map((sender) => {
      ...
      if (existing?.id) matchedContactIds.add(existing.id);
    ...
    // Phase 35: メッセージのないDB登録済みコンタクトも一覧に追加
    if (existingContacts) {
      for (const c of existingContacts) {
        if (matchedContactIds.has(c.id)) continue;
        const cChannels = (c.contact_channels && Array.isArray(c.contact_channels)) ? c.contact_channels : [];
        const channelTypes = [...new Set(cChannels.map((ch: { channel: string }) => ch.channel))];
        const primaryAddress = cChannels.length > 0 ? cChannels[0].address : '';
        contacts.push({
          id: c.id,
          name: c.name || '',
          ...
          activeChannels: cChannels.map((ch: { channel: string; address: string }) => ({
            channel: ch.channel,
            name: ch.address,
          })),
        });
