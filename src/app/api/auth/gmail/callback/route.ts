...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(`${APP_URL}/settings?error=gmail_denied`);
    }

    if (!code || !state) {
      return NextResponse.redirect(`${APP_URL}/settings?error=gmail_invalid`);
    }

    if (!GOOGLE_CLIENT_ID || !GOOGLE_CLIENT_SECRET) {
      return NextResponse.redirect(`${APP_URL}/settings?error=gmail_not_configured`);
    }
    ...
    if (!tokenResponse.ok) {
      ...
      return NextResponse.redirect(`${APP_URL}/settings?error=gmail_token_failed`);
    }
    ...
    if (sb) {
      ...
      if (dbError) {
        ...
        return NextResponse.redirect(`${APP_URL}/settings?error=gmail_save_failed`);
      }
    }

    return NextResponse.redirect(`${APP_URL}/settings?success=gmail_connected`);
  } catch (error) {
    console.error('Gmail OAuthコールバックエラー:', error);
    return NextResponse.redirect(`${APP_URL}/settings?error=gmail_callback_failed`);
  }
}
