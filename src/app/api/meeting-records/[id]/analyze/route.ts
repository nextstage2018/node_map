...
- 必ず有効なJSONを返してください`,
    ...
    // P1-1: サマリー＋意思決定＋タスク＋未確定事項を含むリッチcontent
    ...
    try {
      ...
      if (analysisResult.summary) {
        contentParts.push(analysisResult.summary);
      }

      // 意思決定
      if (analysisResult.new_decisions && analysisResult.new_decisions.length > 0) {
        contentParts.push('');
        contentParts.push('【意思決定】');
        for (const d of analysisResult.new_decisions) {
          contentParts.push(`・${d.title}${d.decision_content ? ` — ${d.decision_content}` : ''}`);
        }
      }
      ...
      if (analysisResult.action_items && analysisResult.action_items.length > 0) {
        contentParts.push('');
        contentParts.push('【タスク】');
        for (const a of analysisResult.action_items) {
          const assignee = a.assignee ? `（${a.assignee}）` : '';
          contentParts.push(`・${a.title}${assignee}`);
        }
      }

      // 未確定事項
      if (analysisResult.new_open_issues && analysisResult.new_open_issues.length > 0) {
        contentParts.push('');
        contentParts.push('【未確定事項】');
        for (const oi of analysisResult.new_open_issues) {
          contentParts.push(`・${oi.title}`);
        }
      }

      const richContent = contentParts.join('\n') || record.title;

      // 既存チェック（同一meeting_record_idで既にあれば更新、なければ挿入）
      // ★ .single() は0件でエラーになるため .limit(1) のみ使用
      ...
      if (existingEvents && existingEvents.length > 0) {
        // 既存あり → 更新
        businessEventId = existingEvents[0].id;
        await supabase.from('business_events')
          .update({ content: richContent, updated_at: new Date().toISOString() })
