...
// GET ?turnId=xxx → task_conversations から該当ターンの前後を返す
// GET ?taskId=xxx&around=timestamp → 指定時刻付近の会話を返す（フォールバック）
// v9.0: seed_conversations テーブル DROP済み。タスク会話のみ対応
...
export async function GET(request: NextRequest) {
  try {
    ...
    // turnId 指定: task_conversations を探索
    if (turnId) {
    ...
    return NextResponse.json(
      { success: false, error: 'turnId または taskId + around パラメータが必要です' },
      { status: 400 }
    );
  } catch (error) {
