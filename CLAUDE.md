...
> V2全9フェーズ + v3.0〜v3.4 + v4.0〜v4.5 + v5.0 + v6.0 + v7.0 + v7.1 + v8.0(Phase1-3) + v9.0 + v10.0〜v10.6 + P2-3(Phase1) + NodeAI(MVP+Phase2) 実装済み。作業開始前に必ず読んでください。
...
| 9 | ~~サーバーサイド日付のJSTズレ~~ | → **dateUtils.ts で解決**: 全サーバーサイドファイルで `toISOString().split('T')[0]` を `getTodayJST()` / `toJSTDateString()` に置換 | 解決 |
| 10 | ~~プロジェクトログDoc重複追記~~ | → **projectLogDoc.service.ts で解決**: 事前アジェンダ二重挿入防止 + AI解析結果の置換方式 | 解決 |
...
| 12 | ~~カレンダーイベント重複作成バグ~~ | → **修正済み**: Cronで `metadata.calendar_event_id` 存在チェック追加（RRULE作成済みならスキップ）+ JSTタイムゾーン修正 + POST側の24時間オーバーフロー防止。既存重複イベントはGoogle Calendarから手動削除が必要 | 解決 |
