...
> V2全9フェーズ + v3.0〜v3.4 + v4.0〜v4.5 + v5.0 + v6.0 + v7.0 + v7.1 + v8.0(Phase1-3) + v9.0 + v10.0〜v10.3 実装済み。作業開始前に必ず読んでください。
...
| # | 課題 | 詳細 | 優先度 |
|---|---|---|---|
| 1 | ~~ログインユーザー別トークン分離~~ | → **v10.3で解決**: `sendChatworkMessage()`にuserId引数追加。`_currentToken`グローバル変数廃止→`chatworkFetchWithToken()`で明示的にトークンを渡す方式に変更。`/api/messages/send`・`/api/messages/reply`・`/api/jobs/[id]/execute`の全呼び出し元でuserId渡し対応。BOT送信（Webhook応答・通知）は環境変数トークン維持 | 解決 |
| 2 | ~~Chatwork通知の403エラー~~ | → **v10.3で解決**: チャネル追加時にBOT自動参加（Slack: conversations.join/invite、Chatwork: PUT /rooms/members）。`botChannelJoin.service.ts` 新設。`/api/projects/[id]/channels` POST後に自動実行 | 解決 |
| 3 | ~~カレンダーイベントへのアジェンダ自動注入（Phase 4）~~ | → **v10.3で解決**: `injectAgendaToCalendarEvents()`を3経路検索に改修（①meeting_records ②recurring_rules ③Calendar API直接検索）。繰り返しイベントのインスタンスID取得にも対応 | 解決 |
| 4 | トークン期限切れ通知なし | Google refresh_tokenの無効化やChatworkトークン再発行時、ユーザーに通知されない。ダッシュボードに接続ステータス表示を検討 | 低 |
