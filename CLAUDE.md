...
最終更新: 2026-03-05（秘書ファースト Phase A〜C + Phase B拡張 + Calendar連携 + ブリーフィング強化 + Calendar×タスク/ジョブ統合 + Google Drive連携 + Drive実運用対応（Phase 44a-44d）+ マルチチャネル・URL・格納指示・ビジネスログ自動蓄積（Phase 45a-45c）+ ナレッジ自動構造化（Phase 47）+ バグ修正・機能強化（Phase 48）+ タスクページ改善・秘書タスク連携（Phase 49）+ タスクファイル添付・AI構想会話改善（Phase 50）+ データ連携強化（Phase 51）+ 組織自動レコメンド（Phase 52）+ 秘書AI総合改善（Phase 53）+ ナレッジページ改善（Phase 57）+ ジョブ再設計・社内相談・署名・文体学習・アカウント紐づけ（Phase 58/58a/58b）+ UX改善・インボックス高速化（Phase 59）+ AI会話パーソナライズ（Phase 61）+ インボックス即時実行UX・カレンダー日程調整・Drive添付保存（Phase 62）まで反映）
...
| Google Calendar API（scheduleMode時のみ） | findFreeSlots()で7日分の空き時間取得→全空き時間を日ごとに提示指示 |
...
### 新規ファイル
- `src/app/api/drive/save-attachments/route.ts` — メッセージ添付ファイルをDriveに保存するAPI。messageIdを受け取り、チャネル別（Gmail/Slack/Chatwork）に添付DL→Drive一時フォルダアップロード→AI分類→ステージング登録

### 変更ファイル
- `src/lib/types.ts` — AiDraftRequestに`scheduleMode?: boolean`追加
- `src/app/api/ai/draft-reply/route.ts` — scheduleModeフラグ対応。trueの場合、findFreeSlots()で空き時間取得→AIプロンプトに実データ注入。全空き時間を日ごとに提示する指示
- `src/services/calendar/calendarClient.service.ts` — findFreeSlots()改善: 今日の営業時間終了後はスキップ、cursorを`Math.max(dayStartMs, nowMs)`で現在時刻以降のみ計算
- `src/components/inbox/ReplyForm.tsx` — `scheduleMode?: boolean`プロパティ追加、AI下書きAPI呼び出し時にscheduleModeフラグを送信
- `src/components/inbox/MessageDetail.tsx` — MessageInlineActions即時実行化、GroupDetail/EmailThreadDetail/SingleMessageDetailの全ビューでscheduleMode伝達、saveToDriveをsave-attachments APIに変更
...
### 3つのメッセージ詳細ビューとアクション表示

| ビュー | 対象 | アクション表示方式 |
|---|---|---|
| GroupDetail | Slack/CWグループチャット | 相手のバブルクリック → アクションチップ表示 |
| EmailThreadDetail | メールスレッド | 下部固定エリアにチップ表示（compact） |
| SingleMessageDetail | 単発メッセージ | 下部固定エリアにチップ表示（compact） |

各ビューで `scheduleMode` state を管理し、日程調整ボタン押下時に `scheduleMode=true` を ReplyForm に伝達。

### findFreeSlots 改善点
```typescript
// 修正前: 今日20:20でも9:00-18:00の空きを返してしまう
let cursor = dayStartMs; // 常に9:00から開始

// 修正後: 現在時刻以降のみ + 営業時間終了日はスキップ
const nowMs = Date.now();
if (dayEndMs <= nowMs) continue; // 営業時間終了日スキップ
let cursor = Math.max(dayStartMs, nowMs); // 今日は現在時刻以降のみ
```

### 重要な実装ノート
- **scheduleMode はサーバーサイドで処理**: クライアントはフラグを送るだけ。カレンダートークンはサーバーサイドにあるため、findFreeSlots()はサーバーで呼ぶ
- **カレンダー未接続時フォールバック**: isCalendarConnected()=falseの場合、「相手に候補日時を聞く形」のフォールバック指示
- **Drive保存はステージング経由**: 即時保存だが最終フォルダには入らない。秘書の「届いたファイル確認」で承認後に最終配置（既存Phase 44フローを再利用）
- **社内相談のみジョブ維持**: consult種別は非同期（相談→回答→AI返信）のためジョブとして残す。それ以外（reply/schedule/todo）は即時実行に移行
