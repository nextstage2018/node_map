...
| # | 課題 | 詳細 | 優先度 |
|---|---|---|---|
| 1 | Chatwork通知の403エラー | BOTアカウントが対象ルームに未参加。Chatwork管理画面でBOTを招待する必要あり（コード修正不要） | 中 |
| 2 | ~~Chatworkタスク作成のto_ids未設定~~ | → **v10.2で解決**: 全ログインユーザーのChatwork account_idを`contact_channels`に登録済み | 解決 |
| 3 | ~~milestonesテーブルのカラム不足~~ | → **v10.2で解決**: `success_criteria`カラム追加済み。analyze APIの`due_date`→`target_date`修正済み | 解決 |
| 4 | ~~Cronからのanalyze自動呼び出しタイムアウト~~ | → **v10.2で解決**: `vercel.json`でsync-meeting-notes/analyze APIの`maxDuration`を300秒に延長（Proプラン） | 解決 |
| 5 | ~~git add -Aで復活した不要ファイル~~ | → **v10.2で解決**: thinking-logs関連ファイル（2ファイル）を削除。他の廃止ファイルは残存なし確認済み | 解決 |
| 6 | ~~谷口さんの重複コンタクト~~ | → **v10.2で解決**: Steady側を削除済み。detect APIのログインユーザー優先マッチで今後は再発しない | 解決 |
