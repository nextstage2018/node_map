...
> V2全9フェーズ + v3.0〜v3.4 + v4.0〜v4.5 + v5.0 + v6.0 + v7.0 + v7.1 + v8.0(Phase1-3) + v9.0 + v10.0〜v10.6 + P2-3(Phase1) + NodeAI(MVP+Phase2+Phase3+Phase4) + v11.0 実装済み。作業開始前に必ず読んでください。
...
- **⚠️ contact_channels に user_id カラムは存在しない**: カラムは `id`, `contact_id`, `channel`, `address`, `frequency` の5つのみ。INSERTに `user_id` を含めるとエラー
- **⚠️ メンバー検出はログインユーザー優先マッチ（v10.2）**: detect APIは `user_service_tokens` から全ログインユーザーのSlack ID（`token_data.authed_user_id`）/ Chatwork ID（`token_data.account_id`）を取得し、`contact_persons.linked_user_id` で確定的にコンタクトを解決。名前照合より前に実行されるため、自社メンバーの重複が原理的に起きない
- **⚠️ 外部メンバーの organization_id は自動セットしない（v10.2）**: detect APIで新規コンタクト作成時、`organization_id: null`、`relationship_type: 'client'` で作成。組織名は手動で設定してもらう
- **⚠️ Chatworkトークン保存時に account_id 自動取得（v10.2）**: POST `/api/settings/tokens` でChatworkトークン保存時、`/v2/me` APIを呼んで `account_id` を `token_data` に自動追加。既存トークンの一括補完は `POST /api/settings/tokens/enrich-chatwork`
...
- **⚠️ contact_persons.name_reading**: TEXT型。読み仮名（TTS用）。NodeAI contextBuilderが `project_members` JOIN経由で自動収集し、AIプロンプトに注入。メンバータブのプロフィール編集で登録
...
- **⚠️ タスク詳細パネルの編集（v10.4）**: 依頼者はドロップダウン、担当者はタグ方式（複数対応）、プロジェクトはドロップダウンで即時API保存。`TaskDetailPanel` に `projects`・`assignees`・`myContactId`・`onTaskUpdate` propsが必要
- **⚠️ 複数担当者（task_assignees）**: 1タスクに複数担当者を紐づけ可能。API: `GET/POST/DELETE /api/tasks/[id]/assignees`。POST body: `{ contact_id }`、DELETE query: `?contact_id=xxx`。`tasks.assigned_contact_id` はメイン担当者（1人目）として互換維持。メイン担当者削除時は次の担当者が自動昇格。UIはタグ方式（`+`で追加、`×`で削除）。カンバンカードには最大3人のバッジ表示（4人以上は`+N`）
...
- **⚠️ Chatwork/Slackトークン分離（v10.3）**: `sendChatworkMessage(roomId, body, userId?)` — userId指定時はユーザー個別トークン（`user_service_tokens`）、未指定時はBOTトークン（環境変数）。`sendSlackMessage` も同様のパターン。ユーザー操作（返信・送信）はuserId必須、BOT操作（Webhook応答・通知・定期配信）はuserId省略。`_currentToken` グローバル変数は廃止済み
...
- **⚠️ JST日付ヘルパー（dateUtils.ts）**: サーバーサイドの日付生成は必ず `src/lib/dateUtils.ts` のヘルパーを使うこと。`new Date().toISOString().split('T')[0]` はVercel(UTC)環境でJSTと1日ズレるため禁止。`getTodayJST()` / `toJSTDateString(date)` / `getJSTNow()` / `addDaysJST(n)` を使用
- **⚠️ プロジェクトログDoc重複防止**: `insertPreMeetingAgenda()` は同日セクション既存時にスキップ。`appendPostMeetingResults()` は既存AI解析結果を削除してから新結果を挿入（再解析時の重複追記を防止）
- **⚠️ プロジェクトログDoc書式設定**: Google Docs APIの `batchUpdate` でテキスト挿入後に `updateTextStyle` を適用。日付ヘッダー=14pt太字、セクション見出し（▶）=12pt太字、カテゴリ見出し（【】）=太字、セパレータ=灰色
- **⚠️ プロジェクトログDoc再作成**: 重複が溜まった場合、`UPDATE projects SET log_document_id = NULL, log_document_url = NULL WHERE id = 'PJ_ID'` で次回Cronが新Doc作成
...
### v9.0: ダッシュボード（3カード + 未確認議事録）

ホーム画面（`/`）を3カード型ダッシュボードに完全置き換え。v10.6で未確認議事録カードを追加（件数0なら自動非表示）。

| カード | コンポーネント | 主な機能 |
|---|---|---|
| 未確認議事録（v10.6） | `PendingMeetingCard.tsx` | 推定PJ判定の議事録をユーザーに確認してもらう（0件なら非表示） |
...
使用API（新規バックエンド不要、既存を活用）:
  ...
  未確認議事録: GET /api/meeting-records/pending, POST /api/meeting-records/pending
```
...
| 振り返り | /analytics | tasks, contact_persons, projects, task_conversations（タスク分析ダッシュボード: 6セクション） |
...
| テーブル | 用途 | ID型 | 注意 |
|---|---|---|---|
| `contact_persons` | コンタクト本体 | TEXT | 手動生成。linked_user_id でアカウント紐づけ。name_reading TEXT（読み仮名・TTS用） |
...
| `tasks` | タスク | UUID | milestone_id / project_id / due_date / scheduled_start/end / completed_at(v11.0) |
...
| `/api/cron/check-token-health` | 毎日 22:00 | トークンヘルスチェック + チャネル通知（v10.4） |
...
v10.1 会議メモ自動取り込み強化（毎時間Cron・マルチユーザー・description PJ判定・analyze API連携・PJ共有リソース化）← 実装済み
v10.2 メンバー管理強化（ログインユーザー優先マッチ・Chatwork account_id自動取得・チャネルアイコン表示）← 実装済み
...
NodeAI Phase 2（応答品質強化: 初回グリーティング・ElevenLabs Flash・60分バッファ・分析的質問検知・読み仮名ガイド） ← 実装済み
NodeAI Phase 3（バグ修正+PJ紐付け: セッション自動終了・lazyFlush 10秒化・お礼/挨拶フィルタ強化・カレンダーPJ選択UI） ← 実装済み
NodeAI Phase 4（会話品質改善+URL直接参加: multi-turn messages・暗黙リクエスト変換・聞き返し禁止・Meet URL直接入力UI） ← 実装済み
v10.6 未確認議事録保留システム（推定PJ判定を保留→ダッシュボード確認→analyzeパイプライン実行） ← 実装済み
v11.0 タスク分析ダッシュボード（振り返り: 週間サマリー/今日のタスク/タスク推移チャート/期限ステータス/メンバー別進捗/PJ別進捗） ← 実装済み
```
...
- **⚠️ sync-meeting-notes の5段階PJ判定 + 未確認保留システム（v10.6）**: ① description埋め込み → ② recurring_rules逆引き（[NM-Meeting]タイトルマッチ） → ②.5 会議タイトルからプロジェクト名・チャネル名で部分一致照合 → ③ 参加者メール（全員自社メールの場合はinternal組織の最新PJを返す） → ④ 最新PJフォールバック（**internal組織のみ**）。**①②のみ `projectConfirmed=true`（自動配信）。②.5③④は推定のため `metadata.needs_project_review=true` で保留→ダッシュボードで確認後にanalyzeパイプライン実行**。参加者が空でもフォールバックは必ず実行（`resolveLatestProject` 独立関数化）
- **⚠️ 未確認議事録ダッシュボードカード（v10.6）**: `PendingMeetingCard.tsx` がダッシュボード上部に表示（件数0なら自動非表示）。`GET /api/meeting-records/pending` で `metadata->>needs_project_review='true'` の議事録を取得。プロジェクト選択→「確定」ボタンで `POST /api/meeting-records/pending` → プロジェクト更新＋needs_project_review解除＋analyzeパイプライン実行
- **⚠️ 会議録のプロジェクト変更**: MeetingRecordList.tsxに移動ボタン（ArrowRightLeft）を実装。PJ判定が誤っていた場合、検討ツリータブから手動で正しいPJに移動可能。PUT `/api/meeting-records/[id]` に `project_id` 対応済み
- **⚠️ tasks.completed_at（v11.0）**: タスク完了時に自動セット。未完了に戻すとNULLクリア。既存完了タスクはマイグレーションで `updated_at` をバックフィル済み。振り返りダッシュボードの完了日判定に使用
...
## v10.1 会議メモ自動取り込み強化（実装済み）

### 概要

Gemini会議メモの自動取り込みを大幅強化。毎時間実行・マルチユーザー対応・プロジェクト判定改善・統一パイプライン連携・プロジェクト共有リソース化。

### 変更内容

**1. Cron頻度を毎時間に変更**
- `vercel.json`: `0 7 * * *`（1日1回）→ `0 * * * *`（毎時間）
- スキャン範囲: 48時間 → 3時間（取込済みはsource_file_idでスキップ）
- 会議終了後、最大1時間以内に自動取り込み

**2. マルチユーザー対応**
- `ENV_TOKEN_OWNER_ID`単一ユーザー → `user_service_tokens`（gmail, is_active, calendar scope）から全ユーザーを自動取得
- 各ユーザーのカレンダーを順番にスキャン
- 同一会議が複数ユーザーのカレンダーにあっても、source_file_idで重複チェックし1回だけ取込

**3. プロジェクト判定改善（3段階フォールバック）**
- ① カレンダーイベントのdescriptionから `project_id:` を正規表現で抽出（最も確実）
- ② 参加者メール → `contact_channels` → `contact_persons` → `organization` → `projects`
- ③ 最新プロジェクト（最終フォールバック）

**4. 統一パイプライン連携**
- Cronからanalyze APIを直接呼び出し（旧: 独自Geminiパーサー → 新: Claude AI解析）
- ミドルウェア認証バイパス: `x-cron-secret`ヘッダー + `cron_secret` URLパラメータの両方対応
- analyze API内でCron用認証バイパス追加（bodyの`user_id`で認証）

**5. プロジェクト配下APIのuser_idフィルタ除去**
- `projects/[id]/channels/route.ts`: GET/POST/DELETE
- `projects/[id]/route.ts`: PUT/DELETE
- `projects/[id]/members/route.ts`: GET/POST/DELETE
- `projects/[id]/messages/route.ts`: プロジェクト確認（inbox_messagesのuser_idは維持）
- 他ユーザーが作成したPJのチャネル・メンバーが全員に表示されるように

### ⚠️ 注意事項

- **⚠️ Cronからのanalyze API呼び出し**: Vercel内部fetchの制約あり。CronのmaxDuration(120秒)内にanalyze APIが完了しないとタイムアウト。現在は外部URL経由のfetch方式。タイムアウトが頻発する場合はanalyzeロジックのサービス関数切り出しを検討
- **⚠️ ミドルウェア認証バイパス**: Edge Runtimeで`process.env.CRON_SECRET`が読めない場合に備え、URLパラメータ`cron_secret`でもバイパス可能にした（middleware.ts）
...
メンバー検出のログインユーザー優先マッチ、Chatwork account_id自動取得、メンバーカードへのチャネルアイコン表示。

### 設計思想

```
【旧（v10.1以前）】
  チャネルメンバー検出 → contact_channels逆引き → 名前照合（曖昧）→ 新規作成
  問題: 外部PJで自社メンバーが別組織のコンタクトとして重複作成される

【新（v10.2）】
  チャネルメンバー検出 → user_service_tokens でログインユーザー確定マッチ（最優先）
  → ログインユーザーなら linked_user_id 経由で既存 contact_persons を確定利用
  → ログインユーザーでなければ外部メンバーとして処理（organization_id=null, relationship_type=client）
```
...
| 優先度 | 段階 | 方法 | 対象 |
|---|---|---|---|
| **0（最優先）** | ログインユーザーマッチ | `user_service_tokens.token_data.authed_user_id/account_id` → `contact_persons.linked_user_id` | 自社メンバー |
| 1 | アドレス照合 | `contact_channels.address` で逆引き | 全員 |
| 2 | 名前照合 | グローバル名前一致（全contact_persons対象） | フォールバック |
| 3 | 新規作成 | `relationship_type='client'`、`organization_id=null` | 外部メンバーのみ |

### 変更ファイル

| ファイル | 変更内容 |
|---|---|
| `src/app/api/projects/[id]/members/detect/route.ts` | ログインユーザー優先マッチ。名前照合グローバル化。organization_id自動セット廃止 |
| `src/app/api/projects/[id]/members/route.ts` | GETレスポンスに `channels` 配列追加（全チャネル情報） |
| `src/components/project/ProjectMembers.tsx` | メンバーカードにSlack/Chatwork/Emailアイコン表示 |
| `src/app/api/settings/tokens/route.ts` | Chatworkトークン保存時に `/v2/me` で `account_id` 自動取得 |
| `src/app/api/settings/tokens/enrich-chatwork/route.ts` | 既存Chatworkトークンの `account_id` 一括補完 + `contact_channels` 自動登録 |

### ⚠️ 注意事項

- **⚠️ contact_channels に user_id カラムは存在しない**: INSERTに含めるとエラー。カラムは `id`, `contact_id`, `channel`, `address`, `frequency` の5つのみ
- **⚠️ linked_user_id の型**: UUID型。`user_service_tokens.user_id`（TEXT型）と比較する場合は `::text` キャストが必要（SQLの場合）。Supabase JSクライアントでは自動変換される
- **⚠️ Chatwork account_id**: `token_data.account_id` に文字列として格納。`contact_channels.address` にも同じ文字列で登録
- **⚠️ enrich-chatwork は冪等**: 既にaccount_idがある場合はスキップ。contact_channelsも既存チェック後にINSERT

---

## NodeAI MVP — 会議中リアルタイムAIアシスタント（Phase 1-2 実装済み）

### 概要

Google Meet会議中にRecall.aiボットが参加し、「ノード」等のトリガーワードを検知してプロジェクト情報をClaude AIで応答、ElevenLabs TTSで音声出力する。

### アーキテクチャ

```
Google Meet → Recall.ai Bot（東京リージョン）→ Deepgram Streaming（日本語ASR）
→ Webhook（transcript.data）→ トリガー検知 or 会話継続モード
→ Claude Haiku応答生成 → ElevenLabs TTS → Recall.ai Audio Output → 会議音声
```
...
| ファイル | 用途 |
|---|---|
| `src/services/nodeai/recallClient.service.ts` | Recall.ai API（Bot作成・停止・音声出力）。リージョン: `ap-northeast-1` |
| `src/services/nodeai/triggerDetector.service.ts` | トリガーワード検知（日本語音声認識の揺らぎ対応） |
| `src/services/nodeai/sessionManager.service.ts` | セッション管理（Supabase。会話バッファ・参加者・応答履歴）。インメモリキャッシュ |
| `src/services/nodeai/sessionCache.service.ts` | インメモリセッションキャッシュ（utteranceバッファ60分・応答履歴・会話継続モード管理） |
| `src/services/nodeai/contextBuilder.service.ts` | プロジェクトコンテキスト構築（タスク・決定事項・未確定事項・MS・上長FB・読み仮名ガイド） |
| `src/services/nodeai/responseGenerator.service.ts` | Claude Haiku応答生成（短い質問: max 120 tokens / 分析的質問: max 300 tokens） |
| `src/services/nodeai/ttsService.ts` | ElevenLabs TTS（eleven_flash_v2_5、日本語最適化設定） |

### トリガー検知の仕様

```
日本語音声認識（Deepgram）の揺らぎが大きいため、緩い検知を採用:
  「ノード」単体が最も安定 → メイントリガー
  「ノードさん」→ のうどさん / ノーズさん / ノートさん / 直登さん / 大登山 / 濃度君 / 弟さん / 能登さん / 野戸さん
  「NodeAI」→ node ai / ノード ai
  「濃度」「直登」「能登」等の漢字誤認識もトリガー

初回トリガー時: AI応答は生成せず、固定グリーティング「起動しました。音声で指示ください。」を返す
→ その後60秒間の会話継続モードに移行

誤検知リスクより検知漏れの方がUXが悪い → 検知は緩く設定
```

### 会話継続モード

```
初回トリガー応答後、60秒間はトリガーワードなしでも応答する:
  ① トリガーワード検知 → 固定グリーティング → last_response_at 記録 → 会話継続モード開始
  ② 60秒以内の発言 → トリガー不要、発言全体が質問として処理
  ③ フィラー（えー/あの/はい等）は無視
  ④ 60秒経過で通常モードに復帰

エコー防止: 自分の音声出力が再度マイクに拾われるのを防止（8秒クールダウン）
```

### 発話バッファ（60分）

```
会議全体の議論をカバーするため、utteranceバッファを60分（直近40発言）保持:
  - sessionCache.service.ts が Vercel warm instance のインメモリMapで管理
  - buildLocalRecentContext() で直近40発言を「直近の議論」としてAIプロンプトに注入
  - 会議中の「さっきの話を整理して」等の振り返り質問に対応可能
```

### 分析的質問の検知と動的レスポンス長

```
isAnalyticalQuestion() で以下のパターンを検知 → max_tokens を動的に切替:
  - タスク整理・確認・洗い出し → 300 tokens
  - 矛盾・食い違い・変更点の検知 → 300 tokens
  - 未確定事項・宿題の整理 → 300 tokens
  - それ以外の簡潔な質問 → 120 tokens

会議アシスタントとしてのユースケース:
  ① タスク化の確認（対象会議の会話を振り返って）
  ② 意思決定事項との矛盾点（過去DBとの比較）
  ③ 未確認事項の整理と把握（過去DBとの比較）
```

### 読み仮名ガイド（TTS精度向上）

```
AIがテキスト応答を生成する際に、固有名詞・人名の正しい読み方を認識:
  データソース（2つ、contextBuilderで統合）:
    ① projects.metadata.pronunciation_guide — プロジェクト名・専門用語
       形式: [{ text: "NextStage", reading: "ネクストステージ" }]
    ② contact_persons.name_reading — メンバー個人名の読み仮名
       形式: TEXT（例: "すずき しんじ"）

  管理UI:
    - 個人名: メンバータブ > プロフィール展開 > 「読み仮名（TTS用）」フィールド
    - プロジェクト名: プロジェクト新規作成時の「読み仮名（TTS用・任意）」フィールド
    - 既存プロジェクト: PUT /api/projects/[id] の pronunciationGuide パラメータ

  注入先: responseGenerator.service.ts の buildSystemPrompt() に「読み方ガイド」セクション
```

### 公開レベル（relationship_type分岐）

```
internal: 全情報回答可能（未確定事項・上長フィードバック含む）
client/partner: 未確定事項・上長フィードバックは非公開。決定事項・タスク進捗のみ
```

### ⚠️ 注意事項

- **Recall.ai リージョン**: `ap-northeast-1`（東京）。URLは `https://ap-northeast-1.recall.ai/api/v1`
- **Recall.ai 認証**: `Token ${RECALL_API_KEY}`（Bearerではない）
- **Deepgram Streaming パラメータ制限**: Recall.ai経由では `language`, `model`, `api_key` のみサポート。`endpointing`, `smart_format`, `utterance_end_ms` は未サポートのため除外（追加するとトランスクリプトが停止する）
- **ElevenLabs モデル**: `eleven_flash_v2_5`（速度と品質のバランス型）。`eleven_turbo_v2_5`（最速・低品質）からの変更で日本語品質が大幅向上。voice_settings: `stability: 0.6`, `similarity_boost: 0.7`, `style: 0.2`
- **Webhook payload**: `data.bot.id`（× `data.bot_id`ではない）、timestampは `{relative, absolute}` オブジェクト
- **エコー防止はepoch時間で比較**: Recall.aiのrelativeタイムスタンプは使わない。`Date.now()/1000` で比較
- **会話継続ウィンドウ60秒**: ログで33秒後のフォローアップが30秒ウィンドウに収まらなかった実績から拡大
- **Webhookは公開パス**: middleware.tsで `/api/nodeai/webhook` を認証バイパス
- **CalendarWidget UI**: Google Meetイベントに「AI参加」ボタン表示。参加中は「停止」ボタン
- **⚠️ contact_persons.name_reading**: TEXT型。SQLマイグレーション `sql/nodeai_name_reading.sql` の実行が必要
- **⚠️ 初回トリガーは固定グリーティング**: `webhook/route.ts` の Step 5 で `responseHistory.length === 0` を判定。AI呼び出しをスキップして「起動しました。音声で指示ください。」を直接TTS送信
- **⚠️ セッション自動終了（Phase 3）**: `autoEndStaleSession()` がWebhook受信時に `started_at` をDBで検証し、3時間超過なら自動ended化。sessionManager.tsの `cleanupStaleSessions()` はCron未登録だったためWebhookトリガーで代替
- **⚠️ lazyFlush間隔 10秒（Phase 3）**: 30秒→10秒に短縮。Vercelインスタンス切替時のutterance喪失リスクを軽減
- **⚠️ お礼・挨拶フィルタ強化（Phase 3）**: `isGreetingOrThanks()` に「しました」「ですね」「素晴らしい」等の短い相槌・感想パターンを追加。複合パターン（「はい了解です」等）にも対応
- **⚠️ AI参加時のPJ選択UI（Phase 3）**: CalendarWidgetでdescriptionにproject_idが埋め込まれていない場合、プロジェクト選択ドロップダウンを表示。「PJなしで参加」も選択可能。プロジェクト一覧は `/api/projects/list-all` から取得
- **⚠️ multi-turn messages（Phase 4）**: 過去3ペアのQ&AをClaude APIの`messages`配列に直接追加。代名詞（「それ」「この件」）の文脈解決が大幅向上。システムプロンプトの「過去の応答」セクションは廃止（重複排除）
- **⚠️ 暗黙リクエスト変換（Phase 4）**: `resolveImplicitRequest()` が会話継続モードで短い発言を明示的リクエストに変換。「未確定」→「未確定事項を教えてください」、「タスク」→「タスクの状況を教えてください」、「決定事項」→「決定事項を教えてください」等
- **⚠️ 聞き返し禁止（Phase 4）**: システムプロンプトの最重要ルールとして「何についてですか？」等の聞き返しを明示的に禁止。文脈から推測して答える指示を徹底。NG例にも追加
- **⚠️ max_tokens調整（Phase 4）**: 通常120→150、会話継続モード用200（新設）、分析質問300→350。会話モードは文脈を踏まえた応答に余裕を持たせる
- **⚠️ Meet URL直接入力（Phase 4）**: CalendarWidgetヘッダーのBotアイコンからURL入力フォームを表示。外部ドメインの会議にもGoogle MeetのURLを貼り付けてNodeAIを参加可能。PJ選択ドロップダウン付き（任意）。参加中は緑パルス＋停止ボタン表示
- **⚠️ isAnalyticalQuestion拡張（Phase 4）**: 「決定事項」「タスクの状況」「マイルストーン」「詳しく」「全体状況」等のパターンを追加。分析的質問検知の精度向上

---

## v11.0 タスク分析ダッシュボード（実装済み）

### 概要

チーム全体のタスク消化状況を俯瞰できる「振り返り」ダッシュボードを `/analytics` に新設。メンバー選択・期間（週/月）切替・ナビゲーション付き。

### 6セクション構成

| # | セクション | コンポーネント | 内容 |
|---|---|---|---|
| ① | 期間サマリー | `WeeklySummaryCards.tsx` | 作成タスク / 完了タスク / AI相談タスク（3指標） |
| ② | 今日のタスク | `DailyTaskSummary.tsx` | 作成（依頼/担当）/ 完了（依頼/担当）の4行 |
| ③ | タスク推移 | `WeeklyTaskChart.tsx` | 曜日別棒グラフ（CSSベース、recharts不要） |
| ④ | 期限ステータス | `DeadlineStatusCards.tsx` | 期限設定/未設定/予定通り/期限切れ（4カテゴリ） |
| ⑤ | メンバー別進捗 | `MemberProgressTable.tsx` | 全メンバー横比較テーブル（プログレスバー付き） |
| ⑥ | プロジェクト別進捗 | `ProjectProgressTable.tsx` | PJ別の完了率・残タスク |

### API

| エンドポイント | パラメータ | 用途 |
|---|---|---|
| `GET /api/analytics/tasks` | `period`(week/month), `date`(YYYY-MM-DD), `contact_id` | 6セクション分の集計データ一括返却 |

### フィルタバー

- **メンバー選択**: `contact_persons`（`linked_user_id IS NOT NULL`）。デフォルト「全員」
- **期間切替**: 週 / 月
- **ナビゲーション**: ← 前 | 今週（今月） | 次 →

### 追加カラム

- `tasks.completed_at` TIMESTAMPTZ — タスク完了日時。`status/route.ts` で `done` 設定時に自動セット、未完了に戻す時にNULLクリア
- SQLマイグレーション: `sql/v11.0_analytics_migration.sql`

### ⚠️ 注意事項

- **棒グラフはCSSベース**: rechartsを使わずTailwind CSSのdivで描画。追加パッケージ不要
- **完了日判定**: `completed_at` 優先、なければ `updated_at` にフォールバック（バックフィル済み）
- **メンバーフィルタ**: `linked_user_id IS NOT NULL` のコンタクトのみ（ログインユーザー = チームメンバー）
- **全員表示がデフォルト**: チーム俯瞰が目的のため、個人フィルタではなく全員表示を初期状態に
...
| # | 課題 | 詳細 | 優先度 |
|---|---|---|---|
| 1 | ~~ログインユーザー別トークン分離~~ | → **v10.3で解決** | 解決 |
| 2 | ~~Chatwork通知の403エラー~~ | → **v10.3で解決** | 解決 |
| 3 | ~~カレンダーイベントへのアジェンダ自動注入（Phase 4）~~ | → **v10.3で解決** | 解決 |
| 4 | ~~トークン期限切れ通知なし~~ | → **v10.4で解決** | 解決 |
| 5 | ~~Chatworkタスク作成のto_ids未設定~~ | → **v10.2で解決** | 解決 |
| 6 | ~~milestonesテーブルのカラム不足~~ | → **v10.2で解決** | 解決 |
| 7 | ~~Cronからのanalyze自動呼び出しタイムアウト~~ | → **v10.2で解決** | 解決 |
| 8 | 既存プロジェクトのBOT参加状況確認 | v10.3のBOT自動参加は新規チャネル追加時のみ。既存PJのチャネルはメンバータブを開けばBOT参加状態が表示される。未参加ならチャネル削除→再追加でBOT自動招待される | 低 |
| 9 | ~~サーバーサイド日付のJSTズレ~~ | → **dateUtils.ts で解決** | 解決 |
| 10 | ~~プロジェクトログDoc重複追記~~ | → **projectLogDoc.service.ts で解決** | 解決 |
| 11 | P2-3 Phase 2〜4（検討ツリー分離・アジェンダ分離・BOT連携） | meeting_groupsテーブル・APIは準備済み。Phase 2: analyze/route.ts ステップ11修正 + DecisionTreeViewグループ切替UI。Phase 3: meetingAgenda.service.ts グループ対応。Phase 4: BOT・通知連携 | 中 |
| 12 | ~~カレンダーイベント重複作成バグ~~ | → **v10.5で解決** | 解決 |
| 13 | 既存PJ読み仮名管理UI | 既存プロジェクトの `pronunciation_guide` をブラウザから編集するUI未実装。現状はAPIのみ（`PUT /api/projects/[id]` の `pronunciationGuide`）。プロジェクト設定画面に読み方ガイド管理セクションを追加予定 | 低 |
| 14 | ~~議事録の他社PJ誤配信~~ | → **v10.6で解決**: 5段階PJ判定 + 未確認保留システム。①②のみ自動配信、②.5③④は保留→ダッシュボード確認 | 解決 |
| 15 | ~~議事録プロジェクト移動UIが動作しない~~ | → **v10.6で解決**: `/api/projects/list-all` 新設。organizations APIのリレーション依存を排除。確認ダイアログ・エラー表示・移動先再解析を追加 | 解決 |
