...
> V2全9フェーズ + v3.0〜v3.4 + v4.0〜v4.5 + v5.0 + v6.0 + v7.0 + v7.1 + v8.0(Phase1-3) + v9.0 + v10.0〜v10.3 + v10.4 実装済み。作業開始前に必ず読んでください。
...
| 4 | ~~トークン期限切れ通知なし~~ | → **v10.4で解決**: 3層通知（設定画面ヘルスチェック + ダッシュボードバナー + 日次Cron→チャネル通知）。全サービス×全ユーザー対応 | 解決 |
