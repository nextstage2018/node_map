...
> V2全9フェーズ + v3.0〜v3.4 + v4.0〜v4.5 + v5.0 + v6.0 + v7.0 + v7.1 + v8.0(Phase1-3) + v9.0 + v10.0〜v10.1 実装済み。作業開始前に必ず読んでください。
...
| 2 | Chatworkタスク作成のto_ids未設定 | 担当者のChatwork account_idが`contact_channels`から解決できていない。メンバーのChatworkアカウント登録が必要 | 高 |
...
| 6 | 谷口さんの重複コンタクト | contact_personsに谷口さんが2レコード（NextStage所属 + Steady所属）。Steady側を削除すべき | 低 |
