...
最終更新: 2026-02-26（Phase 40c まで反映）
...
// getSupabase() → anon key。通常の読み書きに使用。RLSの影響を受ける
// createServerClient() → service role key。RLSをバイパス。Cron等の管理処理用

// ★重要: 同じテーブルへの読み書きは同じクライアントを使うこと
// getSupabase() で INSERT したデータは getSupabase() で SELECT する
// createServerClient() で INSERT したデータが getSupabase() で読めない可能性あり（RLS）
```
...
### 🔴 致命的バグ: 種ボックスの保存が動作しない
- **症状**: 種ボックス（/seeds）から種を追加しても保存されない
- **推定原因**: `TaskService.createSeed()` が `getSupabase()`（anon key）を使用しており、RLS（Row Level Security）が有効な場合に INSERT が失敗する可能性。エラーはサーバーログにのみ出力される
- **調査ポイント**:
  1. ブラウザの開発者ツール（Network タブ）で `/api/seeds` POST のレスポンスを確認
  2. Vercel / ローカルのサーバーログで `Error creating seed` を検索
  3. Supabase ダッシュボードで `seeds` テーブルの RLS ポリシーを確認
  4. RLS が原因の場合: `createServerClient()` を使うか、RLS ポリシーを調整
- **関連ファイル**: `src/services/task/taskClient.service.ts` の `createSeed` メソッド（882行目付近）

### 🟡 種→タスク変換後にタスクが表示されない（修正済み・要確認）
- Phase 40c で convert API を `TaskService.createTask()` 経由に変更済み
- ただし上記の種保存バグが解消しないと検証不可

### 🟡 プロジェクト紐づけで種が登録できない（修正済み・要確認）
- `createSeed` のリトライで `project_id` も除外するよう修正済み
- 上記の種保存バグと関連する可能性あり
