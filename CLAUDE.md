...
> V2全9フェーズ + v3.0〜v3.4 + v4.0〜v4.5 + v5.0 + v6.0 + v7.0 + v7.1 + v8.0(Phase1-3) + v9.0 + v10.0〜v10.2 実装済み。作業開始前に必ず読んでください。
...
| # | 課題 | 詳細 | 優先度 |
|---|---|---|---|
| 1 | Chatwork通知の403エラー | BOTトークンのルームアクセス権限不足。`CHATWORK_BOT_API_TOKEN`の権限確認が必要 | 高 |
| 2 | ~~Chatworkタスク作成のto_ids未設定~~ | ~~担当者のChatwork account_idが`contact_channels`から解決できていない~~ → **v10.2で解決**: 全ログインユーザーのChatwork account_idを`contact_channels`に登録済み | ~~高~~ 解決 |
| 3 | milestonesテーブルのカラム不足 | `due_date`と`success_criteria`カラムが未追加。MS自動登録が失敗中。SQLマイグレーション実行が必要 | 高 |
| 4 | Cronからのanalyze自動呼び出しタイムアウト | Cron(120秒)内にClaude AI解析が完了しない場合がある。手動curlでは成功。analyzeロジックのサービス関数化を検討 | 中 |
| 5 | git add -Aで復活した不要ファイル | seeds/themes/thinking-logs等のv9.0廃止済みファイルがリポジトリに復活。削除コミットが必要 | 低 |
| 6 | ~~谷口さんの重複コンタクト~~ | ~~contact_personsに2レコード~~ → **v10.2で解決**: Steady側を削除済み。detect APIのログインユーザー優先マッチで今後は再発しない | ~~低~~ 解決 |
| 7 | トークン期限切れ通知なし | Google refresh_tokenの無効化やChatworkトークン再発行時、ユーザーに通知されない。ダッシュボードに接続ステータス表示を検討 | 中 |
