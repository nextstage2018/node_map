...
- **⚠️ タスク完了通知の強化**: Block Kitカード更新に加え、元スレッドに完了通知テキスト＋関連資料（drive_documents）リンクを投稿。Chatworkも同様。source_typeがslack/chatwork以外（会議録・手動作成等）の場合はproject_channels経由でPJチャネルに通知
