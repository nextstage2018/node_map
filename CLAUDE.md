...
- **Gemini会議メモ自動取り込み（v6.0）**: Cron（`sync-meeting-notes`）が48時間以内のGoogle Meetイベントをスキャン → 添付 or Drive検索でGemini Docs検出 → Docs APIでテキスト取得 → Geminiパーサー（AI不要）で解析 → meeting_records + パイプライン実行
...
データフロー:
  ...
  3. Cron（sync-meeting-notes）が過去48時間のイベントをスキャン
  4. 会議メモ検出（2段階）:
  ...
  6. geminiParser がテキストパース → AnalysisResult 形式に変換（AI不要）
  7. meeting_records に保存 + 既存パイプライン実行（ビジネスイベント・タスク提案・検討ツリー等）
...
| `src/app/api/cron/sync-meeting-notes/route.ts` | Cronジョブ: 会議メモ自動取り込み（48時間スキャン） |
...
- **Cronスキャン範囲は48時間**: Gemini Docsが添付されるまでのタイムラグ対応。24時間では取りこぼしが発生する
...
会議パイプライン完了後に、プロジェクトのSlack/Chatworkチャネルへ自動投稿。会議終了後の共有作業を完全自動化。MeetGeekは廃止し、Gemini会議メモに一本化。

### 設計思想

```
【旧フロー（v6.0以前）】
  会議終了 → 議事録自動取込 → NodeMap内で確認 → 手動で関係者に共有

【新フロー（v7.0）】
  会議終了 → Gemini会議メモ自動取込 → AI解析 → チャネルに自動投稿
  → Slackならボタンでタスク承認/編集
...
| ファイル | 変更内容 |
|---|---|
| `src/app/api/meeting-records/[id]/analyze/route.ts` | ステップ10追加: チャネル自動投稿 |
