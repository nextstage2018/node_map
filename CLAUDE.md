...
> V2全9フェーズ + v3.0〜v3.4 + v4.0〜v4.5 + v5.0 + v6.0 + v7.0 + v7.1 + v8.0(Phase1-3) + v9.0 + v10.0〜v10.5 + P2-3(Phase1) + NodeAI(MVP+Phase2) 実装済み。作業開始前に必ず読んでください。
...
| 12 | **カレンダーイベント重複作成バグ** | 定期イベント作成時にRRULE付きGoogleカレンダーイベントを作成（POST recurring-rules）＋ process-recurring-rules Cronが同じルールに対して個別イベントをさらに作成 → 二重登録。修正方針: `metadata.calendar_event_id` が存在するルールはCronでのカレンダー登録をスキップ。既存重複イベントはGoogle Calendar APIで削除が必要 | **高** |
