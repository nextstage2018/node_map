...
| 38 | 送信メッセージDB保存・スレッド統合表示・送信済みフィルタ | mainにマージ予定 |
...
1. ~~**送信メッセージの保存**~~: Phase 38 で対応済み（direction カラム追加、送信・返信メッセージのDB保存、送信済みフィルタ）。ただし外部サービス（Chatwork/Slack）の過去の送信メッセージ取得は未対応
2. **送信メッセージのAIコミュニケーション分析連携**: Phase 38b として、送信メッセージもAI分析対象に含めて双方向のコミュニケーション分析を実現する
3. **auto生成コンタクト同士の連絡先結合**: 現状は DBに登録済みコンタクト（confirmed: true）のみ結合可能。isAutoGenerated: true 同士の統合は未実装
4. **ビジネスログの活動履歴連携**: business_events の contact_id が未設定のため、コンタクト詳細の活動履歴タブにビジネスイベントが表示されない。多対多（1イベント複数参加者）の設計見直しも必要
