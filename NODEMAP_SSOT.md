...
| 2026-02-19 | グラフ描画ライブラリはD3.jsに確定 | React Flowより低レベル制御が可能。フォースレイアウト・ズーム・ドラッグ・クラスター円描画に最適 |
| 2026-02-19 | ノード形状を種別で区別 | キーワード=丸、人物=ダイヤモンド、プロジェクト=四角。一目で識別可能 |
| 2026-02-19 | 理解度をサイズ＋色で表現 | 認知=小グレー、理解=中青、習熟=大緑。ノードの成長を直感的に把握 |
| 2026-02-19 | 比較モードは2人並列表示 | 設計書の要件通り。同じタスクで異なるユーザーの思考マップを左右に並べて表示 |
...
### Phase 5（思考マップUI）完了 → 次ステップへの引き継ぎ
...
- **思考マップUIの設計：**
  ...
  - エッジの太さで重み（共起頻度）を表現
  - クラスターは半透明の円で囲む（構想面=青点線、結果面=緑実線）
- **操作フロー：**
  - ベース状態：全ノードをグレーで表示
  - タスク選択→構想ボタン：構想面のノードに色がつき、円で囲まれる
  - 経路ボタン：タスクに関連するエッジが強調表示される
  - 結果ボタン：結果面のノードに色がつき、別色の円で囲まれる
  - ユーザー切替：他ユーザーのマップに切替可能
  - 比較モード：2人のマップを左右に並列表示
- **注意点：**
  - D3.jsのSVG操作はuseEffectの中で直接行っている（React制御外）
  - ツールチップはbody直下にappendしてcleanup時にremoveする
  - ズームはd3.zoomで実装、ドラッグはd3.dragで実装
  - 比較モード時のグラフサイズは画面幅の半分に自動調整

---

## 次にやること

1. **APIキーの準備**（実運用開始前に必要）
   - Gmail API / Slack Bot Token / Chatwork APIトークン / Anthropic APIキー / Supabase

2. **Supabase接続**（本番化に必要）
   - 各サービスのインメモリストアをSupabaseに切り替え
   - マイグレーション実行（001〜003のSQLスキーマ適用）

3. **実データでの検証**
   - 実メッセージ・タスクを使った思考マップの蓄積テスト
   - 理解度判定・クラスター差分の精度検証

4. **UI改善候補**（フィードバック次第）
   - グラフアニメーション（モード切替時のトランジション）
   - ノード検索・フィルタリング機能
   - タイムライン表示（時系列でノード成長を追う）
